...
export default function UpdatePossessionPage() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    try {
      const updateResponse = await fetch(`https://patrimoine-economique-std23044.onrender.com/possession/${libelle}`, {
        method: 'PUT',
      ...
      if (libelle !== currentLibelle) {
        const deleteResponse = await fetch(`https://patrimoine-economique-std23044.onrender.com/possession/${libelle}`, {
          method: 'DELETE',
        });
        ...
        const createResponse = await fetch(`https://patrimoine-economique-std23044.onrender.com/possession`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ libelle: currentLibelle, valeur, dateDebut, dateFin, taux }),
        });
